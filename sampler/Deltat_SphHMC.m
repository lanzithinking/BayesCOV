...
classdef Deltat_SphHMC
    ...
    methods
        % constructor
        function obj=Deltat_SphHMC(q,geom,h,L,alg,reweight)
...
%                 h_adpt.h=obj.init_h();
        ...
        function v=resample_aux(obj,q,r)
            ...
            v=obj.mult_proj(randn(obj.N,obj.dim),q,r);
        end
...
%             q = real(gcirc); v = imag(gcirc).*v_nom;
        ...
        % robuts multiple (Delta) spherical HMC
        function [obj,acpt_ind]=rbt_Dlt_SphHMC(obj,rndL,h,L,r)
            if nargin<2
                rndL=false; h=obj.h; L=obj.L; r=obj.r;
            elseif nargin<3
                h=obj.h; L=obj.L; r=obj.r;
        ...
        % adaptive multiple (Delta) Spherical HMC
        function [obj,acpt_ind]=adp_Dlt_SphHMC(obj,iter,Nadpt)
            % initialization
            q=obj.q; dU=obj.dU;
            v=obj.resample_aux();
...
%                 if any(rand <= (1-mean(q.*obj.q,2)./obj.r^2)./2)
        ...
        function [acpt,time,f_out]=sample(obj,Nsamp,burnrate,thin,SAVE)
            ...
            Niter=Niter+ NBurnIn;
            % allocation to save
            samp=zeros(Nsamp,obj.N,obj.dim);
            engy=zeros(Niter,1);
            ...
            for iter=1:Niter
                ...
                switch obj.alg
                    ...
                    case 'robust'
                        [obj,acpt_ind]=obj.rbt_Dlt_SphHMC();
                    case 'adaptive'
                        [obj,acpt_ind]=obj.adp_Dlt_SphHMC(iter,NBurnIn);
                end
                accp=accp+acpt_ind;
            ...
            if obj.reweight
                samp_diag=samp(:,:,(1:obj.sqdim).*((1:obj.sqdim)+1)./2);
