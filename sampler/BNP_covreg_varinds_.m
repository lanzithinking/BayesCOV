...
for nn=1:numiter
    for ll=1:L  % walk through each row of zeta matrix sequentially
        ...
        for kk=randperm(k);  % create random ordering for kk in sampling zeta_{ll,kk}
            ...
            A_lk_invSig_A_lk = (eta(kk,:).^2).*(M.*(theta(:,ll).^2)'*invSig_vec'-sum(error_nn(ll,:,:),3));
...
mu_tot = zeros(p,N,M);
Omega = zeros(p,k,N,M);
OmegaInvOmegaOmegaSigma0 = zeros(k,p,N,M);
for mm=1:M
    for nn=1:N
        Omega(inds_y(:,nn,mm),:,nn,mm) = theta(inds_y(:,nn,mm),:)*zeta(:,:,nn);
        temp = (Omega(inds_y(:,nn,mm),:,nn,mm)*Omega(inds_y(:,nn,mm),:,nn,mm)'...
            + Sigma_0(inds_y(:,nn,mm),inds_y(:,nn,mm))) \ eye(sum(inds_y(:,nn,mm)));
        OmegaInvOmegaOmegaSigma0(:,inds_y(:,nn,mm),nn,mm) = Omega(inds_y(:,nn,mm),:,nn,mm)'*temp;
        mu_tot(:,nn,mm) = Omega(:,:,nn,mm)*psi(:,nn);  % terms will be 0 where inds_y(:,nn,mm)=0
    end
end
...
for numIter = 1:numTotIters
    ...
    for kk=randperm(k);  % create random ordering for kk in sampling zeta_{ll,kk} 
        ...
        yTilde = y-mu_tot;
        for mm=1:M
            theta_k = theta_k + diag(squeeze(OmegaInvOmegaOmegaSigma0(kk,:,:,mm))'*yTilde(:,:,mm));
            Ak_invSig_Ak = Ak_invSig_Ak + diag(squeeze(OmegaInvOmegaOmegaSigma0(kk,:,:,mm))'*Omega_kk(:,:,mm));
        end
...
for nn=1:N
    ...
    invSigMat = invSig_vec(ones(k,1),:).*reshape(inds_y(:,nn,:),[1,p,M]);
    zeta_theta_invSig = theta_zeta_n'.*invSigMat;  % zeta_theta_invSig = zeta(:,:,nn)'*(theta'*invSig);

    zeta_theta_invSig_theta_zeta = 0;
    zeta_theta_invSig_y_tilde = 0;
    for mm=1:M
        zeta_theta_invSig_theta_zeta = zeta_theta_invSig_theta_zeta + zeta_theta_invSig(:,:,mm)*theta_zeta_n;
        zeta_theta_invSig_y_tilde = zeta_theta_invSig_y_tilde + zeta_theta_invSig(:,:,mm)*y_tilde_n(:,:,mm);
    end
    
    cholSig_xi_n_trans = chol(eye(k) + zeta_theta_invSig_theta_zeta) \ eye(k);  % Sig_eta_n = inv(eye(k) + zeta_theta_invSig*theta_zeta_nn);   
    m_xi_n = cholSig_xi_n_trans*(cholSig_xi_n_trans'*zeta_theta_invSig_y_tilde);  % m_eta_n = Sig_eta_n*(zeta_theta_invSig*y(:,nn));
...
for pp=1:p
    eta_tilde_p = eta_tilde.*reshape(inds_y(pp,:,:),[N,1,M]);
    
    eta_tilde2 = 0;
    eta_tilde_y = 0;
    for mm=1:M
        eta_tilde2 = eta_tilde2 + eta_tilde_p(:,:,mm)'*eta_tilde_p(:,:,mm);
        eta_tilde_y = eta_tilde_y + eta_tilde_p(:,:,mm)'*y(pp,:,mm)';
    end
    
    chol_Sig_theta_p_trans = chol(diag(phi(pp,:).*tau) + invSig_vec(pp)*eta_tilde2) \ eye(L);
    m_theta_p = invSig_vec(pp)*(chol_Sig_theta_p_trans*chol_Sig_theta_p_trans')*eta_tilde_y;
...
for pp = 1:p
    sq_err = 0;
    for mm=1:M
        for nn=inds_vec(inds_y(pp,:,mm))
            sq_err = sq_err + (y(pp,nn,mm) - theta(pp,:)*zeta(:,:,nn)*eta(:,nn))^2;
        end
    end
