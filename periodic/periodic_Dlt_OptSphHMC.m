% inference of local factor process model using Dlt-Opt and Dlt-SphHMC
...
m=[0,0,0]; V=[1,4,2e-1]; % (m,V) in (log) normal priors for eta_*, (eta=log-rho), * = mu, tau(log-sigma), L
...
% allocation to save
Nsamp=1e4; burnrate=0.5; thin=10;
...
f_name=['periodic_dlt_OptSphHMC_dim',num2str(dim),'_',time_lbl,ifrewt];
